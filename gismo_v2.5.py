# gismo_v2.5.py
# This code controls the motors, reads edge/obstacle sensors, sets the RGB LED, 
# makes the robot "waggle" when the touch sensor is activated, shakes arms
# when an edge is detected, and moves the head servo.
...
head_servo_channel = 14  # Servo 3 for the head
...
# Main program loop
try:
    # Initialize head servo to center position
    set_servo_angle(head_servo_channel, 90) 
    ...
    while True:
        ...
        if left_sensor_value == 1 or right_sensor_value == 1:  # Edge detected (line detected)
            ...
            # Move head 30 degrees 
            set_servo_angle(head_servo_channel, 120)  # Adjust angle as needed
