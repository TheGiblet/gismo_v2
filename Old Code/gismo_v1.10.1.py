...
# control for debugging, and camera functionality. The robot prioritizes edge
# detection, scans for a clear path, avoids objects, backs up and turns when an
# edge is detected, uses the buzzer to play different melodies, provides visual
# feedback with the RGB LED, shakes when touched, displays a smiling face on
# the OLED normally and a sad face when an object or edge is detected, uses
# the IR distance sensor to prevent backing into obstacles, allows
# button-triggered debugging actions (running sensor test code files), and
# takes a picture when an object is detected.
