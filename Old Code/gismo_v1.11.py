...
# control for debugging, camera functionality, and IR remote control. The robot
# prioritizes edge detection, scans for a clear path, avoids objects, backs up
# and turns when an edge is detected, uses the buzzer to play different
...
# Import the lirc library for IR remote control
import lirc
...
try:
    while True:
        ...
        # --- IR Remote Control ---
        code = lirc.nextcode()  # Get IR codes
        if code:
            print("IR code received:", code)
            # Map the received code to actions
            if code[0] == "KEY_UP":
                print("Moving forward")
                set_motor_speed(motor1_in1, motor1_in2, 75)  # Adjust speed as needed
                set_motor_speed(motor2_in1, motor2_in2, 75)
            elif code[0] == "KEY_DOWN":
                print("Moving backward")
                set_motor_speed(motor1_in1, motor1_in2, -50)  # Adjust speed as needed
                set_motor_speed(motor2_in1, motor2_in2, -50)
            elif code[0] == "KEY_LEFT":
                print("Turning left")
                set_motor_speed(motor1_in1, motor1_in2, 50)  # Adjust speed as needed
                set_motor_speed(motor2_in1, motor2_in2, -50)
            elif code[0] == "KEY_RIGHT":
                print("Turning right")
                set_motor_speed(motor1_in1, motor1_in2, -50)  # Adjust speed as needed
                set_motor_speed(motor2_in1, motor2_in2, 50)
            elif code[0] == "KEY_OK":  # Example: Stop on KEY_OK
                print("Stopping")
                set_motor_speed(motor1_in1, motor1_in2, 0)
                set_motor_speed(motor2_in1, motor2_in2, 0)
